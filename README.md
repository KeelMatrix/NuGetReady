...
The 0.1.0 supported release profile has one `on.push.tags` trigger with the literal `v*.*.*` pattern, workflow permissions exactly `contents: read`, one unconditional `ubuntu-latest` validation job with a literal 30-minute timeout, and one unconditional `ubuntu-latest` publish job with a literal 10-minute timeout. The validation job has no OIDC or publishing credential and contains exactly this ordered language: `actions/checkout@v6` at the triggering ref with credentials not persisted; `actions/setup-dotnet@v5` from `global.json`; one direct restore, format, Release build, Release test, and pack command with the documented literal arguments; the pinned tool-path installation and installed-binary check below; then `actions/upload-artifact@v4` for the exact configured package paths. The solution and project operands may vary only as literal repository-relative `.sln`/`.slnx` and `.csproj` paths; additional commands, arguments, steps, environments, conditions, scripts, wrappers, aliases, targets, properties, or actions are unsupported/unproven and blocking. `NuGet.config` must contain only the canonical NuGet.org v3 source.
...
./.nugetready/nugetready check --config nugetready.json --artifacts artifacts/release --format json
```

The exact package set in `artifacts/release` is the supplementary source so NuGetReady can certify its own just-built package; other repositories use the same pinned command and resolve NuGetReady from the controlled NuGet.org source. The literal tool path prevents a global tool, alias, or `PATH` replacement from satisfying the validation step. The check must execute immediately before the upload. The publish job depends only on that successful validation job, has permissions exactly `contents: read` and `id-token: write`, and contains exactly three steps: download that same artifact to `artifacts/release` with `actions/download-artifact@v4`; unconditional `NuGet/login@v1` Trusted Publishing authentication; and one literal `pwsh` `dotnet nuget push` of the configured primary `.nupkg` using only the login step's temporary `NUGET_API_KEY`. Product and .NET CLI telemetry are disabled in both jobs.
