...
The publish job depends only on that successful validation job, has permissions exactly `contents: read` and `id-token: write`, and contains exactly three steps: download the original immutable `validated-release-artifacts` identity to `artifacts/release`; unconditional `NuGet/login@v1` Trusted Publishing authentication; and one literal `pwsh` `dotnet nuget push` of the configured primary `.nupkg` using only the login step's temporary `NUGET_API_KEY`. Validation never re-uploads or transforms the artifact. Product and .NET CLI telemetry are disabled in all three jobs. Environment names are preserved with the target Ubuntu runner's case-sensitive identity: the supported runtime bindings are exactly `KEELMATRIX_NO_TELEMETRY`, `DOTNET_CLI_TELEMETRY_OPTOUT`, and, for validation, `NUGET_PACKAGES=/tmp/nugetready-packages`; the publish step must bind exactly `NUGET_API_KEY` to the login output. Credential-name recognition uses a separate case-folded punctuation-insensitive heuristic and never proves that a process receives a variable with that name.

Producer, validator-acquisition, and validator commands are certified only through the small exact PowerShell command language documented by the profile. It preserves command position, quoting, literal argument bytes, empty arguments, and operators; quoted command heads, quoted path whitespace, grouping, punctuation, escaping, and other unmodeled forms are rejected rather than cleaned into an approved command. A workflow file is parsed completely before release relevance is decided: malformed YAML, duplicate mapping keys, invalid root shapes, and multiple documents are blocking input errors with a safe relative workflow path. `not-applicable` is reserved for successfully parsed input whose non-applicability was established.
