...
NuGetReady provides high-confidence workflow certification only for a closed-world release profile: one literal `v*.*.*` tag trigger; workflow `contents: read`; one unconditional `ubuntu-latest` validation job with a literal 30-minute timeout that has no publishing capability, validates the single exact package set from `nugetready.json`, and immediately uploads the exact paths as `validated-release-artifacts`; and one dependent `ubuntu-latest` publish job with a literal 10-minute timeout and permissions exactly `contents: read` plus `id-token: write`. The credential-bearing job contains only `actions/download-artifact@v4`, unconditional `NuGet/login@v1`, and one literal `pwsh` `dotnet nuget push` of the configured primary package using only the temporary login output. Both jobs disable product and .NET CLI telemetry.
