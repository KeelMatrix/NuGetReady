...
Workflow-policy `pass` applies only to the documented closed-world release profile. The triggering tag must be the exact literal `v` plus the configured version, and the configured artifact filenames, packed nuspec identity, immutable validated artifact, and exact primary package selected for publication must all carry that same identity. The checker evaluates jobs with publication operations or recognized credential capability and every dependency or artifact producer that can influence them. Credential capability includes `secrets`-context values; reusable-workflow `secrets` bindings; and the bounded, case-insensitive binding names `NUGET_API_KEY`, `API_KEY`, `ACCESS_TOKEN`, `AUTHORIZATION`, `PASSWORD`, `SECRET`, and `CREDENTIAL`, with `-` treated as `_`, in workflow/job/step `env` and action/reusable-workflow `with` maps. Unknown behavior inside that boundary blocks as unsupported/unproven; unrelated read-only CI without one of those bindings stays outside release policy.
