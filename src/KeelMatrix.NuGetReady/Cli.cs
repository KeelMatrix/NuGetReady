...
internal static partial class CliParser
{
    ...
    public static string HelpText => """
        ...
        Workflow-policy pass means every publication-relevant element in the applicable workflow was provably inspected and no publication-policy failure was found. Literal repository-local scripts and composite actions are recursively inspected through shell -c/--command, PowerShell -Command/-File, and cmd /c forms with an eight-level and 2 MiB indirect-path bound; direct commands and the allowlisted actions actions/checkout, actions/setup-dotnet, actions/upload-artifact, actions/download-artifact, and NuGet/login are supported. Unallowlisted remote actions, expression/environment-derived paths, missing or unreadable paths, opaque commands, and paths beyond the bound are limited/unproven warnings, never proof of safety or publication. Warnings are non-blocking (exit code 0), while deterministic policy errors remain blocking; workflows outside the applicable policy boundary remain not-applicable.
