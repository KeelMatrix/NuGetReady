...
internal static partial class CliParser
{
    ...
    public static string HelpText => """
        ...
        Workflow-policy pass certifies only the closed-world 0.1.0 release profile: one literal v*.*.* tag trigger; one unconditional ubuntu-latest validation job with timeout-minutes 30; and one dependent publish job with timeout-minutes 10 and exact contents:read/id-token:write permissions. The validation job is an exact ordered language: checkout without persisted credentials, SDK setup, direct restore/format/Release build/Release test/pack, `dotnet tool install KeelMatrix.NuGetReady --version 0.1.0 --tool-path .nugetready --configfile NuGet.config --add-source artifacts/release --no-cache --verbosity minimal`, `./.nugetready/nugetready check --config nugetready.json --artifacts artifacts/release --format json`, then exact upload. Extra validation commands or arguments block as unsupported. The publish job may only download validated-release-artifacts, authenticate with unconditional NuGet/login@v1 Trusted Publishing, and directly push the exact primary .nupkg with the temporary login output. Arbitrary GitHub Actions, shell, composite/reusable workflow, dynamic expression, alias/merge-key, arbitrary MSBuild, wrapper, generated-script, command-resolution, additional release-job, and GitHub Release semantics are not interpreted. A deterministic violation is fail/exit 1. Every unknown or unsupported publication-relevant node is unproven and blocks as error/exit 2; it never defaults to safe or to a warning. Simplify and isolate an unconventional publication path or accept the explicit unsupported result.
