...
internal static partial class CliParser
{
    ...
    public static string HelpText => """
        ...
        Supported hosts are Windows, Linux, and macOS. The installed tool targets net8.0 and requires the .NET 8 runtime. Consumer rehearsal also requires a compatible .NET SDK for restore, build, and tool installation. net5.0 and later unqualified modern .NET targets run; Windows-specific modern targets run only on Windows; non-runnable library target frameworks are build-only, including .NET Standard targets and .NET Framework targets such as net48/net481; unsupported or unavailable framework/tooling cases are error/exit 2. A build-only result proves compilation and package consumption, not execution.
        ...
        Unix child processes run in a dedicated process group with startup readiness coordination, bounded descendant cleanup, and verification on successful completion, timeout, and cancellation. Windows targets run under a suspended-start job-object supervisor. Unconfirmed cleanup is error/exit 2 rather than a trustworthy package failure.
        """.TrimEnd();
