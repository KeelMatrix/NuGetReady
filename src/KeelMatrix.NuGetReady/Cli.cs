...
internal static partial class CliParser
{
    ...
    public static string HelpText => """
        ...
        Workflow-policy pass certifies only the closed-world 0.1.0 release profile: one literal v*.*.* tag trigger and separate unconditional ubuntu-latest producer, fresh-runner validator, and publisher jobs. The producer has timeout-minutes 30, uses exact checkout/SDK/restore/format/Release build/Release test/pack commands, and immediately uploads the exact immutable validated-release-artifacts identity. The 10-minute validator downloads that identity to /tmp/nugetready-artifacts, installs NuGetReady 0.1.0 into /tmp/nugetready-tool through an exclusive source selection, and runs `/tmp/nugetready-tool/nugetready check --config nugetready.json --artifacts /tmp/nugetready-artifacts --format json` as its final step. Normal repositories use only https://api.nuget.org/v3/index.json. NuGetReady's own exact 0.1.0 candidate uses an exact temporary config that maps only the tool ID to the immutable artifact directory and transitive dependencies to NuGet.org; any variation is unsupported/unproven. The 10-minute publish job depends on validation, downloads the original producer artifact, authenticates with unconditional NuGet/login@v1 Trusted Publishing, and directly pushes the exact primary .nupkg with the temporary login output. Arbitrary GitHub Actions, shell, composite/reusable workflow, dynamic expression, alias/merge-key, arbitrary MSBuild outside the isolated producer, wrapper, generated-script, command-resolution, additional release-job, and GitHub Release semantics are not interpreted. A deterministic violation is fail/exit 1. Every unknown or unsupported publication-relevant node is unproven and blocks as error/exit 2; it never defaults to safe or to a warning. Simplify and isolate an unconventional publication path or accept the explicit unsupported result.
