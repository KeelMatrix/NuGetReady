...
- Makes workflow-policy `pass` contingent on provable inspection of every applicable publication-relevant element, with bounded recursive local-script/composite traversal and limited/unproven warnings for derived paths, unallowlisted remote actions, opaque commands, and traversal limits.
- Makes command-level workflow inspection exhaustive: only the documented safe-command allowlist or boundedly proven local indirection is silent; task runners, build/container indirection, aliases/functions, generated configuration, and every other unresolved command are limited/unproven warnings.
