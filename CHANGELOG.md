...
- Proves effective permission and environment overrides, capability-bearing workflow coverage, root configuration identity, authentication order, temporary credential scope, exact artifact continuity, and version-tag-only reachability, while blocking reusable workflows, composite actions, aliases/merge keys, wrappers, generated scripts, dynamic command resolution, and other unmodeled execution mechanisms.
